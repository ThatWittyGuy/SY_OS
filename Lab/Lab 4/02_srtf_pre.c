...
int main() {
    int bt[20], p[20], rt[20], wt[20], tat[20], arrival_time[20];
    int i, j, n, total = 0, totalT = 0, complete = 0, time = 0, min_index;
    ...
    int completion_time[40]; // Adjusted size
    int index = 0;

    while (complete < n) {
        // Find the process with the minimum remaining time
        min_index = -1;
        int min_remaining_time = 9999; // A large number

        for (i = 0; i < n; i++) {
            if (arrival_time[i] <= time && rt[i] < min_remaining_time && rt[i] > 0) {

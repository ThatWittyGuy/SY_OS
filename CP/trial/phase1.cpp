...
void OS::ReadData() {
    int k = 0;
    int i = (IR[2] - '0') * 10 + (IR[3] - '0'); // Calculate memory address
...
void OS::WriteData() {
    ...
    outfile << "\n"; // Ensure a newline after writing
    PrintMemory();
}

// Terminate the program
void OS::Terminate() {
    outfile << "\n\n"; // Ensure termination is clear
}
...
void OS::BranchIfTrue() {
    if (C) {
        int i = (IR[2] - '0') * 10 + (IR[3] - '0'); // Calculate memory address
        IC = i; // Set the Instruction Counter to the branch address
    }
}
